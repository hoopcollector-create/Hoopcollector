...
type AdminTab = 'users' | 'shop' | 'shop_orders' | 'regions' | 'classes' | 'approvals' | 'grade_review' | 'manual_class';

const ClassesManager = () => (
    <div>
        <h2 style={{ fontSize: '1.25rem', fontWeight: 800, marginBottom: '1rem' }}>수업 전체 현황</h2>
        <p style={{ color: 'rgba(255,255,255,0.4)' }}>실시간으로 매칭된 수업과 취소 내역을 모니터링하는 화면이 구현될 예정입니다.</p>
    </div>
);
...
const RegionsManager = () => (
    <div>
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)' }}>서비스 운영 가능 지역(시, 구 별)을 통제합니다.</p>
    </div>
);
...
const ManualClassManager = () => (
    <div>
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)' }}>관리자가 직접 예약을 생성하는 기능입니다.</p>
    </div>
);
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto', color: 'white' }}>
            <div style={{ marginBottom: '3rem' }}>
                ...
                <p style={{ color: 'rgba(255,255,255,0.4)', margin: 0, fontSize: '1.1rem' }}>시스템 전반에 대한 최고 권한 관리 센터입니다.</p>
            </div>

            <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', marginBottom: '2.5rem', paddingBottom: '12px', whiteSpace: 'nowrap', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <TabButton active={currentTab === 'approvals'} onClick={() => handleTabSwitch('approvals')}>코치 신청 승인</TabButton>
