        ...
        <div style={{ padding: '20px' }}>
            ...
            {editingProduct && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                    <div style={{ background: '#000000', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '24px', width: '100%', maxWidth: '800px', maxHeight: '90vh', overflowY: 'auto', padding: '40px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                            <h3 style={{ fontSize: '1.5rem', fontWeight: 900 }}>{editingProduct.id ? '상품 정보 수정' : '새 상품 등록'}</h3>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px' }}>
                            ...
                            <div style={{ display: 'grid', gap: '20px' }}>
                                ...
                                <div>
                                    <label style={labelStyle}>슬러그 (URL 주소)</label>
                                    <input style={inputStyle} value={editingProduct.slug} onChange={e => setEditingProduct({...editingProduct, slug: e.target.value})} placeholder="ex: black-hoodie-2024" />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <textarea style={{...inputStyle, minHeight: '100px'}} value={editingProduct.description} onChange={e => setEditingProduct({...editingProduct, description: e.target.value})} />
                                </div>
                            </div>
                        ...
                        <div style={{ marginTop: '40px', display: 'flex', gap: '10px', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '30px' }}>
                            <button onClick={() => setEditingProduct(null)} style={{ flex: 1, padding: '16px', borderRadius: '14px', background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: 'white', fontWeight: 700, cursor: 'pointer' }}>취소</button>
                            <button onClick={handleSave} disabled={isSaving} className="btn-primary" style={{ flex: 2, padding: '16px' }}>
                                {isSaving ? '저장 중...' : '상품 및 옵션 저장하기'}
                            </button>
                        </div>
                    </div>
                </div>
